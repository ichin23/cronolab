...
class ViewUsers extends StatefulWidget {
  ViewUsers(this.turma, this.admins, this.participantes, this.refresh, {Key? key})
      : super(key: key);
  Turma turma;
  List participantes;
  List admins;
    ...
    return widget.participantes.isNotEmpty
        ? ListView.builder(
            ...
            itemBuilder: (context, i) => ListTile(
                  title: Text(
                    widget.participantes[i]["nome"]?? widget.participantes[i]["id"],
                    style: labelDark,
                  ),
                  trailing: widget.admins.where((element) => element["id"]==widget.participantes[i]["id"]).toList().isNotEmpty  ? null: IconButton(
                    icon: Icon(Icons.add),
                    color: primaryDark,
                    onPressed: () async {
